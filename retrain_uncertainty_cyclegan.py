...
os.makedirs('variants_retraining/uncertainty_cyclegan/', exist_ok=True)
...
def train_single_run(data_module: AI4MARSDataModule, checkpoint_path: str, retrain_index: int, epochs: int = 100):
    ...
    save_path = f"variants_retraining/uncertainty_cyclegan/retrain_{retrain_index}_deeplab/retrained_model_checkpoint.pth"
...
ADDITIONAL_SETS_PATH = "./retraining_sets/uncertainty_cyclegan/"
...
for i in range(4, 11):
    if not os.path.exists(f"./variants_retraining/uncertainty_cyclegan/retrain_{i}_deeplab/retrained_model_checkpoint.pth"):
