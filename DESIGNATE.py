...
class GaGan(object):
    ...
    def __init__(self):
        ...
        self.airsim = airsim
        self.dir = './search_sets/mars/search2_feat/'
    ...
    def retrieveImages(self, x, numSet):
        self.dir = "./search_sets/mars/search2_feat/set" + str(numSet)
    ...
    def searchAlgo(self, n_gen, pop_size, numSet):
        path = "./search_sets/mars/search2_feat/set" + str(numSet)
        ...
        with open('ga_results.csv', 'w', newline='') as csvfile:
            ...
            for pos in MyProblem.archive:
                writer.writerow(pos)
                # shutil.copy(pos['img'], './results/archive/'+os.path.basename(pos["img"]))
...
model.load_state_dict(torch.load("./ai4mars/retraining_results/variants_retraining2/search2_feat/retrain_5_deeplab/retrained_model_checkpoint.pth"))
model.eval()
model.to(device)
def predict_img(f):
...
class GanProblem(pymoo.problems.multi.Problem):
    ...
    def _evaluate(self, X, out, *args, **kwargs):
        path = "./search_sets/mars/search2_feat/set" + str(self.numSet) + "/"
        os.makedirs(path, exist_ok = True) 
        F = []
        import psutil
