...
os.makedirs('variants_retraining/uncertainty/', exist_ok=True)
...
def train_single_run(data_module: AI4MARSDataModule, checkpoint_path: str, retrain_index: int, epochs: int = 100):
    ...
    save_path = f"variants_retraining/uncertainty/retrain_{retrain_index}_deeplab/retrained_model_checkpoint.pth"
...
ADDITIONAL_SETS_PATH = "./retraining_sets/uncertainty/"
...
for i in range(8, 10):
    if not os.path.exists(f"./variants_retraining/uncertainty/retrain_{i}_deeplab/retrained_model_checkpoint.pth"):
