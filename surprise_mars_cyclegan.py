...
class GaGan(object):
    ...
    def __init__(self):
        ...
        self.airsim = airsim
        self.dir = 'D:/search_sets/mars/surprise_cyclegan/'
    ...
    def retrieveImages(self, x, numSet):
        self.dir = "D:/search_sets/mars/surprise_cyclegan/set" + str(numSet)
    ...
    def searchAlgo(self, n_gen, pop_size, numSet, kde):
        path = "D:/search_sets/mars/surprise_cyclegan/set" + str(numSet)
        ...
        MyProblem = GanProblem(n_gen, pop_size, rows, self, DNN, numSet, kde)
        print(MyProblem)
...
class GanProblem(pymoo.problems.multi.Problem):
    def __init__(self, n_gen, pop_size, rows, myGaGan, DNN, numSet, kde, **kwargs):
        self.problemDict = []
        self.n_gen = n_gen
    ...
    def _evaluate(self, X, out, *args, **kwargs):
        path = "D:/search_sets/mars/surprise_cyclegan/set" + str(self.numSet) + "/"
        os.makedirs(path, exist_ok = True) 
        F = []
        import psutil
        ...
        for x in X:
            ...
            if True:
                ...
                t3 = time.time()
                test_image = load_image(save_path)                

                # Get the activation pattern from the specified layer
                test_pattern = get_activation_pattern(deeplab, test_image, layer)

                flattened_test_pattern = flatten_activation(test_pattern)

                # Compute LSA for the test pattern
                lsa_score = compute_lsa(self.kde, flattened_test_pattern)
              
                print('fitness = ', lsa_score)
                ...
                if(proportion[0] == True):
                ...
                elif(lsa_score > lsa_threshold):
                    f = 2
                    print("out of distribution output detected")
                else:
                    f = lsa_score
                    ...
                    if(len(archive2) == 0):
                        ...
                        if(closest_distance > threshold_diversity): #  if PixelAccuracy use '<' if Euclidean use '>'
                            ...
                            closest_image_path = archive2[closest_index]
                            closest_image = load_image(closest_image_path)   
                            closest_pattern = get_activation_pattern(deeplab, closest_image, layer)

                            flattened_closest_pattern = flatten_activation(closest_pattern)

                            f_closest = compute_lsa(self.kde, flattened_closest_pattern)
