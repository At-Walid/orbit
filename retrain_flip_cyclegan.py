...
os.makedirs('variants_retraining/flip_cyclegan/', exist_ok=True)
...
def train_single_run(data_module: AI4MARSDataModule, checkpoint_path: str, retrain_index: int, epochs: int = 100):
    ...
    save_path = f"variants_retraining/flip_cyclegan/retrain_{retrain_index}_deeplab/retrained_model_checkpoint.pth"
...
ADDITIONAL_SETS_PATH = "./retraining_sets/flipping_cyclegan/flipping_cyclegan/"
...
for i in range(7,10):
    if not os.path.exists(f"./variants_retraining/flip_cyclegan/retrain_{i}_deeplab/retrained_model_checkpoint.pth"):
